...
class BankifAIConnection(models.Model):
    ...
    def _compute_expected_expiring_synchronization_date(self):
        for connection in self:
            connection.expected_expiring_synchronization_date = connection.token_datetime + relativedelta(days=int(self.env['ir.config_parameter'].sudo().get_param('account_statement_import_online.days_consent_expected_duration', 90)))
