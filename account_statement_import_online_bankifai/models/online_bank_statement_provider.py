...
class OnlineBankStatementProvider(models.Model):
    ...
    def _bankifai_get_matched_bankifai_account_ids(self):
        ...
        if str2bool(self.env["ir.config_parameter"].sudo().get_param("account_statement_import_online_bankifai.check_callback_url", 'True')):
            bankifai_account_ids = self.bankifai_user_id.bankifai_connection_ids.filtered(lambda connection: connection.callback_url == self.bankifai_callback_url)._get_matched_bankifai_account_ids([self.account_number,self.journal_id.card_number])
        else:
            bankifai_account_ids = self.bankifai_user_id.bankifai_connection_ids._get_matched_bankifai_account_ids([self.account_number,self.journal_id.card_number])
