...
class AccountJournal(models.Model):
    ...
    def _get_action_card_number_wizard(self):
        self.ensure_one()
        action = self.env['ir.actions.act_window']._for_xml_id('account_statement_import_online_bankifai.bankifai_card_number_wizard_action')
        action['context'] = {
    ...
    def action_select_bankifai_bank(self):
        if self.type == 'bank' and not self.bank_account_id:
            return self._get_action_create_bank_account()
        elif self.type == 'credit' and not self.card_number:
            return self._get_action_card_number_wizard()

        if not self.online_bank_statement_provider_id:
            self.online_bank_statement_provider = 'bankifai'
